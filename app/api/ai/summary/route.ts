...
import { REWRITE_SUMMARY } from "@/lib/ai/prompts"

const Cost = 2 // ai_improve_summary
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { resumeId, currentSummary, targetRole, tone } = parsed.data
    ...
    await deductCredits(user.id, "ai_improve_summary")
...
`
    ...
    const responseText = await runQwen3(REWRITE_SUMMARY, messages, 1000)
    ...
    return NextResponse.json({ error: "Failed to generate summary. Credits refunded." }, { status: 500 })
  }
}